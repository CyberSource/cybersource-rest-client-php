...
/**
 * V2paymentsProcessingInformationCaptureOptions Class Doc Comment
 *
 * @category    Class
 * @package     CyberSource
 ...
 * @link        https://github.com/swagger-api/swagger-codegen
 */
class V2paymentsProcessingInformationCaptureOptions implements ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $swaggerModelName = 'v2payments_processingInformation_captureOptions';
     ...
     * @param float $captureSequenceNumber Capture number when requesting multiple partial captures for one payment. Used along with _totalCaptureCount_ to track which capture is being processed.  For example, the second of five captures would be passed to CyberSource as:   - _captureSequenceNumber_ = 2, and   - _totalCaptureCount_ = 5
     * @return $this
     */
    public function setCaptureSequenceNumber($captureSequenceNumber)
    {
        ...
        if (!is_null($captureSequenceNumber) && ($captureSequenceNumber > 99)) {
            throw new \InvalidArgumentException('invalid value for $captureSequenceNumber when calling V2paymentsProcessingInformationCaptureOptions., must be smaller than or equal to 99.');
        }
        if (!is_null($captureSequenceNumber) && ($captureSequenceNumber < 1)) {
            throw new \InvalidArgumentException('invalid value for $captureSequenceNumber when calling V2paymentsProcessingInformationCaptureOptions., must be bigger than or equal to 1.');
        }
    ...
    public function setTotalCaptureCount($totalCaptureCount)
    {
        ...
        if (!is_null($totalCaptureCount) && ($totalCaptureCount > 99)) {
            throw new \InvalidArgumentException('invalid value for $totalCaptureCount when calling V2paymentsProcessingInformationCaptureOptions., must be smaller than or equal to 99.');
        }
        if (!is_null($totalCaptureCount) && ($totalCaptureCount < 1)) {
            throw new \InvalidArgumentException('invalid value for $totalCaptureCount when calling V2paymentsProcessingInformationCaptureOptions., must be bigger than or equal to 1.');
        }
    ...
    public function setDateToCapture($dateToCapture)
    {
        if (!is_null($dateToCapture) && (strlen($dateToCapture) > 4)) {
            throw new \InvalidArgumentException('invalid length for $dateToCapture when calling V2paymentsProcessingInformationCaptureOptions., must be smaller than or equal to 4.');
        }
