...
class PostCustomerRequest implements ArrayAccess
{
    ...
    public function setId($id)
    {
        if (!is_null($id) && (strlen($id) > 32)) {
            throw new \InvalidArgumentException('invalid length for $id when calling PostCustomerRequest., must be smaller than or equal to 32.');
        }
        if (!is_null($id) && (strlen($id) < 1)) {
            throw new \InvalidArgumentException('invalid length for $id when calling PostCustomerRequest., must be bigger than or equal to 1.');
        }
